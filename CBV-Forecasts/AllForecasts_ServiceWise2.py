...
def forecast_arima(service_name, bu_code):
    ...
    forecast_df = pd.DataFrame({
        '2+10 Forecast (ARIMA)': forecast,
        '4+8 Forecast (ARIMA)': forecast  # This can be adjusted if needed
...
def forecast_sarima(service_name, bu_code):
    ...
    forecast_index = pd.date_range(start=service_data.index[-1] + pd.DateOffset(months=1), periods=12, freq='M')
    forecast_df = pd.DataFrame({
        '2+10 Forecast (SARIMA)': forecast,
        '4+8 Forecast (SARIMA)': forecast  # This can be adjusted if needed
    }, index=forecast_index)
...
def generate_forecasts_for_bu(bu_code):
    ...
    if all_forecasts_arima:
        ...
        combined_forecasts_arima.dropna(inplace=True)  # Remove any NaN values
    ...
    if all_forecasts_sarima:
        ...
        combined_forecasts_sarima.dropna(inplace=True)  # Remove any NaN values
