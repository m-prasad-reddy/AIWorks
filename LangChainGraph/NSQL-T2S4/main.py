...
def create_table_hints_cache(schema: Dict[str, Any], cache_path: str = TABLE_HINTS_FILE) -> Dict[str, List[str]]:
    """Generate and cache static table hints (fallback or optional)"""
    Path(".cache").mkdir(exist_ok=True)
    table_hints = {
...
def create_dynamic_table_hints(schema: Dict[str, Any]) -> Dict[str, List[str]]:
    """Generate and cache dynamic table hints using DynamicHintsGenerator"""
...
def build_prompt(user_query: str, schema: Dict[str, Any], selected_tables: List[str]) -> str:
    ...
    return f"""{ddl_section}-- Using valid Transact-SQL, answer this (SELECT only):
-- Use [SCHEMA].[TABLE] format
-- {user_query}
SELECT"""

# Initialize model
tokenizer, model = AutoTokenizer.from_pretrained("NumbersStation/nsql-6B"), AutoModelForCausalLM.from_pretrained("NumbersStation/nsql-6B")
...
def generate_sql(prompt: str) -> str:
    ...
    if not sql.strip().upper().startswith("SELECT"):
        sql = "SELECT " + sql.split("SELECT", 1)[-1] if "SELECT" in sql.upper() else "SELECT * FROM [ERROR]"
...
async def startup_event():
    ...
    if not db_session.current_db and db_session.list_available_dbs():
        # Connect to first available DB by default
...
def main():
    ...
    while True:
        user_input = input("\nEnter your question (exit/reload/use/model): ").strip()
        ...
        if user_input.lower().startswith('use '):
            new_db = user_input[4:].strip()
            try:
                if db_session.connect(new_db):
                    schema = initialize_metadata_phase(db_session.config[new_db])
                    print(f"✅ Switched to {new_db}")
                else:
                    print(f"⚠️ Failed to connect to {new_db}")
            except Exception as e:
                print(f"⚠️ Error: {str(e)}")
            continue
        ...
        if user_input.lower().startswith('model '):
            ...
            try:
                tokenizer, model = AutoTokenizer.from_pretrained(model_name), AutoModelForCausalLM.from_pretrained(model_name)
...
if __name__ == "__main__":
    try:
        ...
        if 'db_session' in globals() and db_session.conn:
            db_session.conn.close()